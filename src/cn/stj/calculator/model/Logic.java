...
import cn.stj.calculator.widgets.CalculatorDisplay.Scroll;
...
public class Logic {
	...
	private static final int INPUT_MAXIMUM_LIMIT = 32; // add by
														// fugy@20140621,for
														// limit maximum input.
	...
	private void clear(boolean scroll) {
		...
		mDisplay.setText("", scroll ? CalculatorDisplay.Scroll.UP
				: CalculatorDisplay.Scroll.NONE, true);
		cleared();
	}
	...
	// add begin by fugy@20140621,for limit maximum input.
	public boolean isOutMaximumlength(String delta) {
		String text = getText();
		if (text.length() + delta.length() > INPUT_MAXIMUM_LIMIT) {
			Toast.makeText(
					mContext,
					mContext.getResources().getString(
							R.string.input_maximum_limit), Toast.LENGTH_SHORT)
					.show();
			return true;
		}
		return false;
	...
	void onEnter() {
		if (mDeleteMode == DELETE_MODE_CLEAR) {
			if (!mContext.getResources().getBoolean(
					R.bool.clear_history_click_enter)) {
	...
	public void evaluateAndShowResult(String text, Scroll scroll) {
		try {
			...
			if (!text.equals(result)) {
				...
				// delete by zhongrenzhan@20140520,bug:1352,do not clear result.
			...
			mDisplay.setText(mResult, scroll);
			// setDeleteMode(DELETE_MODE_CLEAR);//delete by
			// zhongrenzhan@20140520
		}
	}
